...
Longtime 的核心目标是让低成本模型承担日常工作，强模型仅在需要时介入兜底，解决后交回原执行流程。

| 角色 | 预期模型分工 | 职责 |
| --- | --- | --- |
| 周期监督者（LUNA） | 低成本模型 | 查询进度、按控制器路由推进、独立执行验收命令 |
| 执行器（DSH） | 能胜任常规工作的低成本模型 | 调查、修改代码、运行测试、同一 session 续跑 |
| 异常处理者（SOL） | 更强的模型 | 诊断阻塞、修订技术方案、恢复任务并交回执行器 |
| Python 控制器 | 不使用模型 | 校验状态和契约、限制重试、记录事务与副作用意图 |

这是一种部署策略，不是仓库内置的模型价格路由器。DSH 使用其已配置模型，异常处理适配器沿用宿主默认模型或原异常任务模型；使用前应确认这些设置确实对应预期档位。状态升级本身不保证模型能力升级。

强模型兜底不降低验收门槛：SOL 解决技术问题后恢复任务，普通 ACCEPT 仍由 LUNA 在 REVIEW 轮提交。节省目标来自减少昂贵模型的常驻参与和无效重复执行；实际收益需要包含全部角色消耗的对照评测。
...
flowchart TD
  ...
  S[低成本周期监督者] --> P[Python 控制器 begin]
  C --> P
  P --> A{唯一规划动作}
  A -->|DSH_START / CONTINUE / STATUS| I[prepare-intent]
  I --> M[MCP HTTP server]
  M --> B[BridgeRunner]
  B --> D[低成本执行模型 / DSH tools]
  D --> H[DSH_HOME 会话日志]
